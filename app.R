   ...
   # Application title
        ...
        sidebarLayout(
                ...
                sidebarPanel(
                        ...
                        selectInput("ticket",
                                    "Selecciona la columna donde se encuentra el identificador de tus tickets:",
                                    choices = c("Esto","le","toca","a","Ariel"),
                                    selected = NULL,
                                    multiple = FALSE),
                        
                        selectInput("items",
                                    "Selecciona la columna donde se encuentran los Items a analizar:",
                                    choices = c("Esto","le","toca","a","Ariel"),
                                    selected = NULL,
                                    multiple = FALSE),
...
server <- function(input, output) {
    ...
    output$freq <- renderPlot({
        ...
        output$rules <- renderDataTable({
                ...
                transaction <- ddply(data,c("ticket"),function(df1)paste(df1$items, collapse = ","))
                
                write.table(x = transaction,file = tmp_cu <- file(),row.names = FALSE,quote = FALSE)
                
                tr <- read.transactions(file = tmp_cu,format = "basket",sep = ",")
                
                ## rules <- apriori(data = tr, parameter = list(supp=input$supp, conf=input$conf),control = list(verbose = FALSE))
                rules <- apriori(data = tr, parameter = list(supp=0.0001, conf=0.10), control = list(verbose = FALSE))
